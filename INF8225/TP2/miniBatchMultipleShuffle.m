%*************************************************************************%
%FICHIER:     miniBatchMultipleShuffle.m                                  %
%DESCRIPTION: Construit un modèle par apprentissage via la méthode des    %
%               mini-batchs (stochastic gradient descent) en faisant      %
%               des nouveaux mini-batches à chaque itération.             %
%                                                                         %
...
converged = 0;
while ~converged
    % Définition des nouveaux batchs
        [XA, YA, batches] = createMiniBatches(XA, YA, K, Theta);
        % Remise en ordre du vecteur des résultats de l'ensemble d'apprentissage
        % pour le calcul de la précision
        YAvect = zeros(size(YA,1),1);
        for i=1:learningSize
            [~, index] = max(YA(i,:));
            YAvect(i) = index;
...
% save(['experiments/experimentMiniBatchMultipleShuffle_K' int2str(K) '_taux' num2str(taux_dapprentissage,'%.4f') '_seuil' int2str(seuilConvergence) '.mat'])