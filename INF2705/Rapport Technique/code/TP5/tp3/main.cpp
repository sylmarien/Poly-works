...
const unsigned int MAXNPARTICULES = 100000;
...
void avancerParticulesNOTWORKING(unsigned int i)
{
	particules[i].tempsDeVieRestant -= deltaT;

	// Si la particule a encore du temps de vie on gère son mouvement
	if (particules[i].tempsDeVieRestant >= 0) {

		// On retient son altitude intiale en cas de collision pour pouvoir corriger l'effet de la gravité
		float hauteurInitiale = particules[i].position[2];
		...
		// On retient la distance verticale et sa valeur au carré parcourue au cas où on a une collision
		float distanceVerticaleCarre = (particules[i].position[2] - hauteurInitiale) * (particules[i].position[2] - hauteurInitiale);
		float distanceVerticale = sqrt(distanceVerticaleCarre);

		float depassement[3] = { 0.0, 0.0, 0.0 };
		for (unsigned int j = 0; j < 3; j++)
		{
			// vérifier les collisions "negatives" et déplacer la particule à son emplacement correct
			if (particules[i].position[j] <= bMin[j])
			{
				// On fait la symétrie par rapport au plan que l'on a dépassé
				depassement[j] = bMin[j] - particules[i].position[j];
				particules[i].position[j] += 2 * depassement[j];
			}
			// vérifier les collisions "positives" et déplacer la particule à son emplacement correct
			else if (particules[i].position[j] >= bMax[j])
			{
				// On fait la symétrie par rapport au plan que l'on a dépassé
				depassement[j] = bMax[j] - particules[i].position[j];
				particules[i].position[j] += 2 * depassement[j];
			}
		}

		if (depassement[2] != 0.0)
		{
			particules[i].position[2] -= depassement[2]; // On ramène la particule sur le plan de hauteur constante pour calculer sa vraie trajectoire
			particules[i].vitesse[2] = gravite * deltaT * (1 - depassement[2] / distanceVerticale) - particules[i].vitesse[2]; // mise à jour et inversion
			particules[i].position[2] += particules[i].vitesse[2] * deltaT * depassement[2] / distanceVerticale; // deplacement après collision
			particules[i].vitesse[2] += - gravite * deltaT * depassement[2] / distanceVerticale; // actualisation de la vitesse en fin de mouvement après collision
		}
		else
			particules[i].vitesse[2] += -gravite * deltaT;

		// On inverse la vitesse sur les coordonnées correspondantes
		for (unsigned int j = 0; j < 2; j++)
		{
			if (depassement[j] != 0.0)
				particules[i].vitesse[j] = -particules[i].vitesse[j];
		}
	} else // Sinon on la réinitialise
		initPart(particules[i]);
}
